...
public class HarvestPicker : BaseSettingsPlugin<HarvestPickerSettings>
{
    ...
    private async Task FetchPrices()
    {
        ...
        try
        {
            ...
            builder.Path = "/poe1/api/economy/exchange/current/overview";
            ...
            // Десериализуем напрямую в анонимный тип или JObject
            var jObject = Newtonsoft.Json.Linq.JObject.Parse(str);

            var dataMap = new Dictionary<string, double>();

            // Получаем массив items для маппинга id -> имя
            var itemsDict = new Dictionary<string, string>();
            foreach (var item in jObject["items"])
            {
                itemsDict[item["id"].ToString()] = item["name"].ToString();
            }

            // Получаем цены из lines
            foreach (var line in jObject["lines"])
            {
                var id = line["id"].ToString();
                if (itemsDict.TryGetValue(id, out var name))
                {
                    dataMap[name] = (double)line["primaryValue"];
                }
            }

            if (dataMap.Any(x => x.Value is 0) || dataMap.Count < 4)
            {
            ...
            {
                BlueJuiceValue = dataMap.GetValueOrDefault("Primal Crystallised Lifeforce"),
                YellowJuiceValue = dataMap.GetValueOrDefault("Vivid Crystallised Lifeforce"),
                PurpleJuiceValue = dataMap.GetValueOrDefault("Wild Crystallised Lifeforce"),
                WhiteJuiceValue = dataMap.GetValueOrDefault("Sacred Crystallised Lifeforce"),
            };
